...
// 12 Rasi Horoscope API (Reads from uploads/file.json)
app.get('/api/horoscope/rasi', (req, res) => {
  try {
    const filePath = path.join(__dirname, 'uploads', 'file.json');

    if (!fs.existsSync(filePath)) {
      console.warn("Horoscope file not found at " + filePath);
      return res.json({ ok: false, error: 'Horoscope data unavailable' });
    }
    ...
    // Check for nested Gemini structure
    if (Array.isArray(rootJson) && rootJson.length > 0 && rootJson[0].content && rootJson[0].content.parts) {
      try {
        const text = rootJson[0].content.parts[0].text;
        // Remove markdown code blocks
        const cleanJsonStr = text.replace(/```json/g, '').replace(/```/g, '').trim();
        cleanData = JSON.parse(cleanJsonStr);
      } catch (parseErr) {
        console.error("Error parsing inner JSON:", parseErr);
        return res.json({ ok: false, error: 'Invalid data format' });
      }
    } else if (Array.isArray(rootJson)) {
      // Assume it is already clean array
      cleanData = rootJson;
    }

    res.json({ ok: true, data: cleanData });
  } catch (err) {
    console.error("Error serving rasi list:", err);
    res.status(500).json({ ok: false, error: err.message });
  }
});
